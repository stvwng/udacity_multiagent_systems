...
class BookingManager:
    ...
    def add_booking(self, service_type, customer_name, expected_service=None, is_urgent=False):
        ...
        # check if priority handling is required
        if is_urgent:
            special_handling = f"priority handling required for {customer_name}"
...
@tool
def handle_deposit(customer_name: str, is_urgent: bool) -> str:
    """Handles deposit requests.

    Args:
        customer_name: The name of the customer.
        is_urgent: Whether the customer request is urgent
...
@tool
def handle_postal(customer_name: str, is_urgent: bool) -> str:
...
@tool
def handle_loan(customer_name: str, is_urgent: bool) -> str:
...
@tool
def handle_bill_payment(customer_name: str, is_urgent: bool) -> str:
...
@tool
def handle_international_transfer(customer_name: str, is_urgent: bool) -> str:
...
@tool
def handle_general_inquiry(customer_name: str, is_urgent: bool) -> str:
...
class ChineseBankPostOfficeAgent(ToolCallingAgent):
    ...
    def handle_customer_request(self, customer_name: str, request: str, expected_service: str) -> str:
        ...
        # Check for urgency
        urgency_prompt = f"""
        You are an urgency analyzer that determines if a customer request is urgent.
        
        Analyze this customer request: "{request}"
        
        Return ONLY a boolean variable (i.e., "true" or "false"). Do not include any explanation, 
        additional text, or punctuation. 
        """
        
        urgent_request = self.urgency_detector.run(urgency_prompt).strip().lower()
        urgent_request = urgent_request == "true"
        ...
        router_prompt = f"""
        Execute the handle_{service_type} tool with these paramenters:
            customer name: '{customer_name}',
            is_urgent: '{urgent_request}'
...
def analyze_urgency(request: str) -> bool:
    ...
    request_lower = request.lower()
    if "emergency" or "urgent" in request_lower:
        return True
...
class UrgencyDetector(ToolCallingAgent):
    ...
    def __init__(self, model: OpenAIServerModel):
        ...
        super().__init__(
            ...
            description="""You are a urgency analyzer agent for a Chinese Postal Bank. 
            Your job is to identify if a customer's request is urgent.
            
            IMPORTANT: You MUST ONLY use the analyze_urgency tool to classify the request.
            NEVER provide explanations or direct responses to the customer.
            
            When using the analyze_urgency tool, you must identify the request as either urgent or not urgent
